...
ui <- dashboardPage(
    ...
    .tabbable > .nav > li > a {background-color: lightgrey;   color:white}")),
                              tabsetPanel(
                                  ...
                                  tabPanel("Présentation des créateurs de l'application",box(imageOutput("margaux")),box(imageOutput("oriane")),box(imageOutput("marie")),box(imageOutput("clemence"))),
                              ))),
...
et 2015."})
    ...
    output$nb_paniers_text <-renderText({"Pour une année il y a plusieurs points puisqu'un point correspond à chaque panier marqué par un joueur.
La courbe bleue est la tendance du nombre de paniers pour chaque année."})
